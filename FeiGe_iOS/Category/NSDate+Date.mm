...
//  FeiGe
//
//  Created by 曾照成 on 17/3/26.
//  Copyright © 2017年 FeiGe. All rights reserved.
//
 ...
 *  @return 获取上传图片那一刻的时间
 */
+(NSString *)getUploadImageDateString
{
    NSDate *dates = [NSDate date];
    NSDateFormatter *formatter =  [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyyMMddHHmmssms"];
    NSTimeZone* timeZone = [NSTimeZone timeZoneWithName:@"Asia/beijing"];
    [formatter setTimeZone:timeZone];
    //生成日期
    [formatter setDateFormat:@"yyyyMMdd"];
    [formatter setTimeZone:timeZone];
    NSString *strDT = [formatter stringFromDate:dates];
    return strDT;
}

//计算两个时间的间隔(单位秒)
+(NSInteger)gapBetween:(NSString *)beforeTime and:(NSString *)nextTime
{
    beforeTime = [NSDate formattingTimeString:beforeTime];
    
    nextTime = [NSDate formattingTimeString:nextTime];
    ...
    NSDate *date1 = [dateFormatter dateFromString:nextTime];
    
    NSDate *date = [dateFormatter dateFromString:beforeTime];
...
+(NSString *)formattingTimeString:(NSString *)timeString
{
    ...
    if ([timeString rangeOfString:@"+"].location != NSNotFound) {
        ...
        fromatString = [array firstObject];
        if ([fromatString rangeOfString:@"T"].location != NSNotFound) {
            NSArray * array = [fromatString componentsSeparatedByString:@"T"];
            fromatString = [NSString stringWithFormat:@"%@ %@",[array firstObject],[array lastObject]];
        }
    }else if ([timeString rangeOfString:@"."].location != NSNotFound) {
        fromatString = [[timeString componentsSeparatedByString:@"."] firstObject];
    }
